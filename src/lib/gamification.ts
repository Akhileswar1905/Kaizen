 ...
 */
export function calculateStatIncrease(event: GamificationEvent): {
  stat: StatType
  value: number
} {
 ...
 */
export interface RawHabitLog {
  is_completed: boolean
  log_date: string
}
export interface RawDsaCompletion {
  problem_id: string
}
...
export interface RawSystemDesignProgress {
...
export interface RawVarsityProgress {
  module_number: number
...
export interface LogAggregation {
  ...
  systemDesignProgress: RawSystemDesignProgress[]
...
export function derivePlayerStats(
  ...
  logs.systemDesignProgress.forEach(() => {
    const { xp, stat } = XP_MAPPING.SYSTEM_DESIGN_MODULE
    totalXp += xp
    stats[stat] += Math.floor(xp / 10) || 1
  })

  // Process Finance
  logs.financeLogs.forEach(() => {
    const { xp, stat } = XP_MAPPING.FINANCE_LOGGED
    totalXp += xp
    stats[stat] += Math.floor(xp / 10) || 1
  })

  // Process Varsity
  logs.varsityProgress.forEach(() => {
    const { xp, stat } = XP_MAPPING.VARSITY_CHAPTER_COMPLETED
    totalXp += xp
    stats[stat] += Math.floor(xp / 10) || 1
  })
  ...
  // Process Notes
  logs.notes.forEach(() => {
    const { xp, stat } = XP_MAPPING.NOTE_SAVED
    totalXp += xp
    stats[stat] += Math.floor(xp / 10) || 1
  })
